...
* `--exclusion-rules` : (Optional) case sensitive. Project's exclusion rules, separated with comma and no
  ...
  *example* `--exclusion-rules`= "
  ...
  To get the full list do run help on this command (*deliver* --help).</br>
  *REMARKS*: Following exclusion rules are to be used in collaboration with the *JeeFileDiscoverer* extension
...
This command is used to deliver a new version of an application to AIP Console. It <u>doesn't</u> marks that version as
current by default and won't run an analysis.
...
* `--exclusion-rules` : (Optional) case sensitive. Project's exclusion rules, separated with comma and no
  ...
  *example* `--exclusion-rules`= "
  ...
  </br>*REMARKS*: Following exclusion rules are to be used in collaboration with the *JeeFileDiscoverer* extension
  EXCLUDE_JAVA_FILES_WHEN_A_FULL_JEE_PROJECT_EXISTS,
  EXCLUDE_JAVA_FILES_WITH_AN_INCOMPLETE_PACKAGE,
  EXCLUDE_JAVA_FILES_WITH_AN_UNNAMED_PACKAGE,
  EXCLUDE_WEB_JSP_PROJECT_WHEN_JAVA_FILES_EXISTS_FOR_THE_SAME_WEB_XML_FILE,
